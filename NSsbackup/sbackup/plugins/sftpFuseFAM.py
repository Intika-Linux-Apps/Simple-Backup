...
class sftpFuseFAM (pluginFAM)  :
	...
	def getdoc(self):
		doc = "SFTP schema is like : sftp://user:pass@server/anything" 
		return doc
	...
	def __defineMountDirName(self, remote):
		...
		if not match : 
			raise FuseFAMException("Error matching the schema 'sftp://user:pass@server/anything' with '%s' (The '/' after server is mandatory)" % remote)
		else :
			if match.group(2) :
				exp1 = re.compile("^([^:]+?)(:[^@]+?)?@")
