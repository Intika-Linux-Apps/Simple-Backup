...
class ftpFuseFAM (pluginFAM)  :
	...
	def matchScheme(self,remoteSource):
		if re.compile("^(ftp://[^/]+?/)(.*)").search(remoteSource) :
			return True
		return False
	...
	def mount(self,source, mountbase):
		...
		if not os.path.exists(mountpoint) :
			os.mkdir(mountpoint)
		server,pathinside = re.compile("^(ftp://[^/]+?/)(.*)").search(source).groups()
		...
		FAM.delete(errFile)
		if retval != 0 :
			raise FuseFAMException("Couldn't mount '%s' into '%s' : %s" %  (server , mountpoint, errStr))
		
		return (server,mountpoint,pathinside)
	
	def getdoc(self):
		doc = "FTP schema is like : ftp://user:pass@server/anything" 
		return doc
	...
	def __defineMountDirName(self, remote):
		...
		if not match : 
			raise FuseFAMException("Error matching the schema 'ftp://user:pass@server/anything' with '%s' (The '/' after server is mandatory)" % remote)
		else :
			if match.group(2) :
				exp1 = re.compile("^([^:]+?)(:[^@]+?)?@")
				user = exp1.search(match.group(2)).group(1)
				dirname = "ftp_"+user+"@"+match.group(3)
				return dirname
			else :
				dirname = "ftp_"+match.group(3)
				return dirname